...
✅ AI 工作区：6 个模块进了包，197 个单测对【产物里的模块】全过（构建 35069527609）
✅ 欢迎页大标题已删、新标签页 Zen logo 已隐藏
✅ 主菜单可按 pref 配置（默认隐藏打印 / 登录 Firefox / 保存页面）
✅ dsh 0.1.5 接口查清：传输/认证/会话列表全有据可查（docs/dsh-0.1.5-interface.md）
   会话列表客户端已落地（KokoaDshSessions，49 用例）；
   【边界】外壳无法遥控 dsh 切换会话 —— 已证实是产品本地状态，非分支能绕
⏳ 待实机验证：点 AI 工作区 -> dsh 起来 -> 面板打开；分屏；菜单隐藏
⏭  之后再说：AI 工作区侧栏（含会话列表 UI 接入）、branding 图标
```
